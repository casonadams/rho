...
pub use builder::{THOUGHT_SIGNATURES, build_antigravity_request};
...
use builder::is_valid_thought_signature;
...
use rho_sdk::contract::{
    ...
    ModelMetadata, ProviderCapability, ProviderDescriptor, ProviderRequest, ProviderStreamEvent,
};
                                ...
                                if let Some(content) = cand.content {
                                    for part in content.parts {
                                        ...
                                        if let Some(call) = part.function_call {
                                            ...
                                            if let Some(s) = sig
                                                && is_valid_thought_signature(Some(&s))
                                            {
                                                THOUGHT_SIGNATURES
                                                    .lock()
                                                    .unwrap()
