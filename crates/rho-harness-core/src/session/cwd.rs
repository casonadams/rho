use crate::error::{AppError, Result};
use std::path::Path;

pub fn record_session_for_cwd(sessions_dir: &Path, cwd: &Path, session_id: &str) -> Result<()> {
    std::fs::create_dir_all(sessions_dir)?;
    let index_file = sessions_dir.join(".last_sessions.json");
    let mut map: std::collections::BTreeMap<String, String> = if index_file.exists() {
        let content = std::fs::read_to_string(&index_file)?;
        serde_json::from_str(&content).unwrap_or_default()
    } else {
        std::collections::BTreeMap::new()
    };
    let canonical_cwd = std::fs::canonicalize(cwd).unwrap_or_else(|_| cwd.to_path_buf());
    map.insert(canonical_cwd.display().to_string(), session_id.to_string());
    let json = serde_json::to_string_pretty(&map).map_err(|e| AppError::Session(e.to_string()))?;
    std::fs::write(&index_file, json)?;
    Ok(())
}

async fn read_last_sessions_map_async(index_file: &Path) -> std::collections::BTreeMap<String, String> {
    if tokio::fs::try_exists(index_file).await.unwrap_or(false) {
        let content = tokio::fs::read_to_string(index_file).await.unwrap_or_default();
        serde_json::from_str(&content).unwrap_or_default()
    } else {
        std::collections::BTreeMap::new()
    }
}

async fn canonicalize_cwd_async(cwd: &Path) -> String {
    let canonical = tokio::fs::canonicalize(cwd).await.unwrap_or_else(|_| cwd.to_path_buf());
    canonical.display().to_string()
}

pub async fn record_session_for_cwd_async(sessions_dir: &Path, cwd: &Path, session_id: &str) -> Result<()> {
    tokio::fs::create_dir_all(sessions_dir).await?;
    let index_file = sessions_dir.join(".last_sessions.json");
    let mut map = read_last_sessions_map_async(&index_file).await;
    map.insert(canonicalize_cwd_async(cwd).await, session_id.to_string());
    let json = serde_json::to_string_pretty(&map).map_err(|e| AppError::Session(e.to_string()))?;
    tokio::fs::write(&index_file, json).await?;
    Ok(())
}

pub fn last_session_for_cwd(sessions_dir: &Path, cwd: &Path) -> Result<Option<String>> {
    let index_file = sessions_dir.join(".last_sessions.json");
    if !index_file.exists() {
        return Ok(None);
    }
    let content = std::fs::read_to_string(&index_file)?;
    let map: std::collections::BTreeMap<String, String> = serde_json::from_str(&content).unwrap_or_default();
    let canonical_cwd = std::fs::canonicalize(cwd).unwrap_or_else(|_| cwd.to_path_buf());
    Ok(map.get(&canonical_cwd.display().to_string()).cloned())
}

pub async fn last_session_for_cwd_async(sessions_dir: &Path, cwd: &Path) -> Result<Option<String>> {
    let index_file = sessions_dir.join(".last_sessions.json");
    if !tokio::fs::try_exists(&index_file).await.unwrap_or(false) {
        return Ok(None);
    }
    let content = tokio::fs::read_to_string(&index_file).await?;
    let map: std::collections::BTreeMap<String, String> = serde_json::from_str(&content).unwrap_or_default();
    let canonical_cwd = tokio::fs::canonicalize(cwd).await.unwrap_or_else(|_| cwd.to_path_buf());
    Ok(map.get(&canonical_cwd.display().to_string()).cloned())
}
