...
pub(super) struct ContextComparisonReport {
    ...
    pub(super) before: ContextEvaluation,
    pub(super) after: ContextEvaluation,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub(super) struct ContextEvaluation {
    pub(super) model_visible_messages: usize,
    pub(super) model_visible_bytes: usize,
    ...
    pub(super) tool_denials: usize,
    pub(super) usage_available: bool,
}

pub(super) struct ContextEvaluationInput<'a> {
...
pub(super) async fn run_context_evaluation(input: ContextEvaluationInput<'_>) -> ContextEvaluation {
    ...
    let TurnOutput { metrics, usage, .. } = engine
        .run_turn(TurnRequest::new("continue"), &TerminalRenderer::default())
        .await
        .unwrap();
...
pub(super) fn long_context_history() -> Vec<Message> {
    (0..30)
        .flat_map(|index| {
            [
...
pub(super) async fn context_comparison(
    base_dir: &Path,
    before_usage: Usage,
    after_usage: Usage,
