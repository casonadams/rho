...
use crate::plugin::contract::{MessageContent, MessageRole, ModelMessage};
use crate::session::SessionEventKind;
use crate::session::context::context_memory;
use crate::tools::{ApprovalCapability, approval_context};
...
impl AgentEngine {
    pub(super) async fn run_external_turn(
        ...
        let executor = tools.neutral_executor(tool_context);
        ...
        loop {
            ...
            match terminal {
                NeutralTurnTerminal::Checkpoint(pending) => {
                    if !self.config.auto_approve && renderer.prompt_continue_budget(pending.completed_model_turns).await
                    {
                        messages = Vec::new();
    ...
    async fn persist_canonical_history(&self, new_messages: &[ModelMessage]) -> Result<()> {
        ...
        crate::session::context::context_memory(
            self.session_manager.clone(),
            self.config.context_window_messages,
            self.config.compaction_max_bytes,
        )
    ...
    async fn finish_external_turn(&self, artifacts: ExternalTurnArtifacts) -> Result<TurnOutput> {
        ...
        self.session_manager
            ...
            .await?;
        let status = if output.finish_reason == crate::plugin::contract::FinishReason::ContentFilter {
