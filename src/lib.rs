...
use engine::provider::{CredentialStrategy, ProviderId, RigCredentialVerifier};
...
pub async fn run_cli() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(cmd) = cli.command {
        match cmd {
            ...
            Commands::Models => {
                let provider = ProviderId::from_str(&config.provider)?;
                let catalog = engine::provider::list_models(provider, &auth_store, &config.config_dir).await?;
    ...
    // Check one-shot prompt from flag or stdin pipe
    ...
    if let Some(prompt) = prompt_text {
        // One-shot mode
        let intent_spec = ClarificationHandler::process_intent(&prompt, false)?;
        let engine = engine::AgentEngine::new(config, auth_store, cli.resume.as_deref()).await?;
        let renderer = TerminalRenderer::default();

        let res = engine
            .run_turn(
                engine::runner::TurnRequest {
                    prompt: &prompt,
