...
fn apply_editor_action(editor: &mut super::EditorState, action: UiAction) {
    match action {
        UiAction::Insert(value) => editor.insert_char(value),
        ...
        UiAction::DeleteWordBackward => {
            let _ = editor.delete_word_backward();
        }
        UiAction::DeleteWordForward => {
            let _ = editor.delete_word_forward();
        }
        UiAction::DeleteToLineStart => {
            let _ = editor.delete_to_line_start();
        }
        UiAction::DeleteToLineEnd => {
            let _ = editor.delete_to_line_end();
        }
