...
fn running_tool_rolling_tail_truncation_under_massive_chunks() {
    let mut tool = super::RunningTool::new("bash", "seq 1 10000", None);
    for i in 1..=5000 {
        tool.append_chunk(&format!("line {i:04}: detailed execution log output\n"));
    }

    assert!(
        tool.output.len() <= super::MAX_RUNNING_BUFFER_BYTES,
        "tool output length ({}) must not exceed MAX_RUNNING_BUFFER_BYTES ({})",
        tool.output.len(),
        super::MAX_RUNNING_BUFFER_BYTES
    );
    assert!(
        tool.output.len() <= super::MAX_RUNNING_OUTPUT_BYTES + 200,
        "tool output length ({}) should be trimmed close to MAX_RUNNING_OUTPUT_BYTES ({})",
        tool.output.len(),
        super::MAX_RUNNING_OUTPUT_BYTES
    );
...
fn running_tool_preserves_utf8_char_and_newline_boundaries() {
    let mut tool = super::RunningTool::new("bash", "unicode stream", None);
    let line = "🦀 🚀 🔥 ✨ 这是一个很长的多字节测试行 🌟 💫\n";
    for _ in 0..2500 {
        tool.append_chunk(line);
    }

    assert!(tool.output.len() <= super::MAX_RUNNING_BUFFER_BYTES);
    assert!(
...
fn running_tool_single_massive_chunk_bounded() {
    let mut tool = super::RunningTool::new("bash", "massive chunk", None);
...
fn running_tool_massive_line_without_newlines_bounded_safely() {
    let mut tool = super::RunningTool::new("bash", "one line", None);
    let massive_line = "🔥abc🚀def".repeat(15000); // ~150 KB with multi-byte chars
    tool.append_chunk(&massive_line);

    assert!(tool.output.len() <= super::MAX_RUNNING_OUTPUT_BYTES);
