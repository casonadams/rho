...
use crate::engine::provider::{CredentialStrategy, ModelCatalog, ProviderId, curated};
use crate::error::{AppError, Result};
use crate::plugin::capability::{CapabilityError, CapabilityId, PluginId};
...
use crate::plugin::external::{ExternalPlugin, ExternalProvider};
...
impl ProviderRegistry {
    pub async fn load(config: &Config) -> Result<Self> {
        ...
        for candidate in PluginLoader::configured_candidates(&config.config_dir, &config.plugins) {
            ...
            for declaration in &plugin.manifest().capabilities {
                if declaration.id.kind() != crate::plugin::capability::CapabilityKind::Provider {
                    continue;
                }
    ...
    pub fn selected_credential(
        ...
        name: &str,
        store: &AuthStore,
    ) -> Result<Option<(u64, crate::plugin::contract::ScopedCredential)>> {
...
pub fn capability_id(name: &str) -> Result<CapabilityId> {
    format!("provider:{name}")
        .parse()
        .map_err(|error: crate::plugin::capability::CapabilityValidationError| AppError::Provider(error.to_string()))
}
