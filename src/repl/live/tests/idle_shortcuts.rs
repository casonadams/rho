...
#[tokio::test]
async fn test_idle_shortcut_ctrl_s_toggles_block_style_and_persists() {
    let (mut controller, mut session, mut engine) = setup_shortcut_harness().await;
    let dir = std::env::temp_dir().join(format!("rho_test_toggle_{}", uuid::Uuid::new_v4()));
    session.config.config_dir = dir.clone();
    let mut last_escape_time = None;
    let mut batch = LiveBatch::new();
    ...
    handle_shortcut_action(InputAction::BlockStyleToggle, ctx, &mut batch)
        .await
        .unwrap();

    assert_eq!(controller.block_style(), crate::ui::theme::BlockStyle::Border);
    assert_eq!(session.renderer.theme.block_style, crate::ui::theme::BlockStyle::Border);
    assert_eq!(session.config.ui.block_style.as_deref(), Some("border"));

    let toml_str = std::fs::read_to_string(dir.join("config.toml")).unwrap();
    assert!(toml_str.contains("block_style = \"border\""));
    ...
    handle_shortcut_action(InputAction::BlockStyleToggle, ctx2, &mut batch)
        .await
        .unwrap();

    assert_eq!(controller.block_style(), crate::ui::theme::BlockStyle::Solid);
    assert_eq!(session.renderer.theme.block_style, crate::ui::theme::BlockStyle::Solid);
    assert_eq!(session.config.ui.block_style.as_deref(), Some("solid"));
