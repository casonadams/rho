...
pub async fn update_single_plugin(
    ...
    let downloaded = ctx.client.download_asset(&asset.browser_download_url).await?;
    let binary = extract_binary(&asset.name, &downloaded, &spec.executable_name)?;
    write_binary_atomically(&dest_path, &binary)?;

    let mut updated_cfg = plugin_cfg.clone();
    updated_cfg.version = Some(release.tag_name.clone());
    Config::add_plugin_async(ctx.config_dir, name, updated_cfg).await?;
...
pub async fn handle_update_plugin(config: &Config, name: &str) -> rho_harness_core::error::Result<PluginUpdateStatus> {
    ...
    match &status {
        PluginUpdateStatus::AlreadyUpToDate { version } => println!("Plugin '{key}' is already up to date ({version})"),
