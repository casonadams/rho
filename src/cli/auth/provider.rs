...
pub fn prompt_select_auth_method() -> Result<AuthMethod> {
    ...
    let idx = super::terminal::prompt_select("Select authentication method:", &options)?;
    if idx == 0 {
...
pub fn prompt_select_oauth_provider() -> Result<String> {
    ...
    let idx = super::terminal::prompt_select("Select provider to log in:", &items)?;
    Ok(options
        .get(idx)
...
pub fn prompt_select_api_key_provider(config: &Config) -> Result<String> {
    ...
    let items: Vec<String> = options.iter().map(|(id, desc)| format!("{id:<14} [{desc}]")).collect();
    let idx = super::terminal::prompt_select("Select provider to configure:", &items)?;
    Ok(options
        .get(idx)
        .map(|(id, _)| id.to_string())
...
pub fn prompt_auth_method(provider_label: &str) -> Result<AuthMethod> {
    ...
    let msg = format!("Select authentication method for {provider_label}:");
    let idx = super::terminal::prompt_select(&msg, &options)?;
    if idx == 0 {
        Ok(AuthMethod::OAuth)
    } else {
        Ok(AuthMethod::ApiKey)
